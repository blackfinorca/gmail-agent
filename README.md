...
| `OPENAI_MODEL`            | `gpt-4o-mini`      | Model for summaries + invoice extraction |
...
|---|---|
| `.env` | `cp .env.example .env`, then fill in real values (at minimum `OPENAI_API_KEY`). |
...
cp .env.example .env
#   ↳ edit .env and set OPENAI_API_KEY (and any other values you want to change)
